...
class AbstractCrawler(object):
    ...
    def _create_tasks(self, bandwidth):
    ...
    def fetch_mp4_by_m3u8_uri(self, as_mp4):
        ...
        try:
            ...
            if len(key_segments_pairs) > 0:
                checks = self._fetch_segments_to_local_tmpdir(key_segments_pairs) 
                while checks is False:
                    try:
                        self.bandwidth+=1
                        key_segments_pairs = self._create_tasks(bandwidth=self.bandwidth)
                        if len(key_segments_pairs) < 1:
                            raise ValueError('NO FOUND TASKS!\n Please check m3u8 url.')
