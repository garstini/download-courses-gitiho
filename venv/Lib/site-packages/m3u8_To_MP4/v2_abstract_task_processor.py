...
class AbstractFileCrawler(AbstractCrawler):
    def __init__(self, m3u8_uri, m3u8_file_path, customized_http_header=None,
        ...
        super(AbstractFileCrawler, self).__init__(m3u8_uri,
                                                  ...
                                                  max_retry_times,
                                                  num_concurrent, mp4_file_dir,
                                                  mp4_file_name, tmpdir)
...
class AbstractUriCrawler(AbstractCrawler):
    def _request_m3u8_obj_from_url(self, url):
        try:
            response_code, m3u8_bytes = sync_http.retrieve_resource_from_url(
                self.best_addr_info, url,
                customized_http_header=self.customized_http_header)
            if response_code != 200:
                raise Exception(
                    'DOWNLOAD KEY FAILED, URI IS {}'.format(url))
                
            m3u8_str = m3u8_bytes.decode()
            m3u8_obj = m3u8.loads(content=m3u8_str, uri=url)
    ...
    def _get_m3u8_obj_with_best_bandwitdth(self, bandwidth=0):
        m3u8_obj = self._request_m3u8_obj_from_url(self.m3u8_uri_base)
        if m3u8_obj.is_variant:
            best_bandwidth = 9999999999
            best_bandwidth_m3u8_inx = 0

            for playlist in range(len(m3u8_obj.playlists)):
                if m3u8_obj.playlists[playlist].stream_info.bandwidth < best_bandwidth:
                    best_bandwidth = m3u8_obj.playlists[playlist].stream_info.bandwidth
                    best_bandwidth_m3u8_inx = playlist

            logging.info("Choose the best bandwidth, which is {}".format(
                best_bandwidth))
            logging.info("Best m3u8 uri is {}".format(m3u8_obj.playlists[best_bandwidth_m3u8_inx-bandwidth].absolute_uri))

            self.m3u8_uri = m3u8_obj.playlists[best_bandwidth_m3u8_inx-bandwidth].absolute_uri

            m3u8_obj = self._request_m3u8_obj_from_url(self.m3u8_uri)
        else:
    ...
    def _create_tasks(self,bandwidth=0):
        # resolve ts segment uris
        m3u8_obj = self._get_m3u8_obj_with_best_bandwitdth(bandwidth=bandwidth)
